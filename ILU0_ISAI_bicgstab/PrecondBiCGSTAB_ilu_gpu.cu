...
int Batched_BiCGSTAB_ILU_Gpu_helper(const PagedCSRMatrices & A_pages,const PagedVectors& b_pages,PagedVectors & x_pages, 
    SolverResults & solver_results,const bool is_parilu , const int num_iter_par_ilu,  double & PST, double & IST, double & OET  )
{
     ...
     const bool is_parilu , const int num_iter_par_ilu,  double & PST, double & IST, double & OET   )
{
    ...
    success_code = Batched_BiCGSTAB_ILU_Gpu_helper(A_pages,b_pages,x_pages, solver_results, is_parilu, num_iter_par_ilu, PST, IST, OET);
